...
class _MembersAdminPageState extends State<MembersAdminPage> {
  ...
  Future<void> _saveMember(_MemberDraft draft, {required bool isEdit}) async {
    final myCse = (_myCse ?? '').trim();
    if (myCse.isEmpty) return;

    if (draft.cse.trim() != myCse) {
      _showError("Action interdite: CSE différent du vôtre.");
      return;
    }
    ...
    try {
      ...
      if (draft.newPhotoBytes != null && draft.newPhotoExt != null) {
        ...
        // si renommage => l’ancien fichier peut devenir orphelin
  ...
  Future<void> _deleteMember(Map<String, dynamic> row) async {
    ...
    if (rowCse != myCse) {
      _showError("Action interdite: suppression hors de votre CSE.");
      return;
    }
...
class _MemberEditDialog extends StatefulWidget {
  ...
  const _MemberEditDialog({required this.title, required this.initial});
